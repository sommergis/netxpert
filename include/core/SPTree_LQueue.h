...
#ifndef _SPTree_LQueue
 #define _SPTree_LQueue  /* self-identification: #endif at the end of the file */
...
*/
class SPTree_LQueue : public MCFClass, public netxpert::core::ISPTree
{
...
/*--------------------------------------------------------------------------*/
   ...
   SPTree_LQueue( cIndex nmx = 0 , cIndex mmx = 0 , bool Drctd = true );
...
/*--------------------------------------------------------------------------*/
   ...
   SPTree_LQueue::FONumber MCFGetFO( void );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   SPTree_LQueue::FONumber MCFGetFO( Index ND , cIndex_Set DB );
...
inline MCFClass::Index SPTree_LQueue::MCFSNde( cIndex i )
{
 if( DirSPT )
  #if( SPT_STRTN )
   return( Startn[ i ] );
...
inline MCFClass::Index SPTree_LQueue::MCFENde( cIndex i )
{
 if( DirSPT )
...
inline MCFClass::CNumber SPTree_LQueue::MCFCost( cIndex i )
{
 if( DirSPT )
...
inline MCFClass::FNumber SPTree_LQueue::MCFUCap( cIndex i )
{
 #if( ! DYNMC_MCF_SPT )
  cIndex pos = DictM1[ i ];
  #if( SAME_GRPH_SPT )
...
inline MCFClass::FNumber SPTree_LQueue::MCFDfct( cIndex i )
{
 #if( USENAME0 )
  cIndex myi = i;
 #else
...
inline bool SPTree_LQueue::IsClosedArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
...
inline bool SPTree_LQueue::IsDeletedArc( cIndex name )
{
 return( SPTree_LQueue::IsClosedArc( name ) );
 }

/*--------------------------------------------------------------------------*/

inline void SPTree_LQueue::SetOrigin( cIndex NewOrg )
{
...
inline void SPTree_LQueue::SetDest( cIndex NewDst )
{
...
//Johannes Sommer
//inline void SPTree_LQueue::SetDest( int i )
//{
//	if ( i == -1)
//	{
...
inline bool SPTree_LQueue::Reached( cIndex i )
{
...
inline unsigned int* SPTree_LQueue::Predecessors( void )
{
 return( NdePrd );
 }
...
inline MCFClass::Index SPTree_LQueue::DestN( void )
{
 return( NDsts );
 }
...
inline MCFClass::cIndex_Set SPTree_LQueue::Dests( void )
{
 return( DstBse );
 }
...
//Johannes Sommer
inline unsigned int* SPTree_LQueue::ArcPredecessors( void )
{
 CalcArcP();
 return( ArcPrd );
 }
...
//Johannes Sommer
inline void SPTree_LQueue::GetArcPredecessors( unsigned int *outArcPrd )
{
...
//Johannes Sommer
inline void SPTree_LQueue::GetPredecessors( unsigned int *outPrd )
{
	unsigned int size = MCFnmax()+1;
...
//Johannes Sommer
inline void SPTree_LQueue::GetDests( unsigned int *outDstBse )
{
	unsigned int size = MCFnmax()+1;
...
#endif  /* SPTree_LQueue.h included */

/*--------------------------------------------------------------------------*/
/*-------------------------- End File SPTree_LQueue.h ----------------------*/
/*--------------------------------------------------------------------------*/
