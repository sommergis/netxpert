...
SPTree_LQueue::SPTree_LQueue( cIndex nmx , cIndex mmx , bool Drctd )
	:
        MCFClass( nmx , mmx )
{
 DirSPT = Drctd;
...
void SPTree_LQueue::LoadNet( cIndex nmx , cIndex mmx , cIndex pn , cIndex pm ,
...
// Johannes Sommer
 ...
 void SPTree_LQueue::LoadNet(unsigned int nmx, unsigned int mmx, unsigned int pn, unsigned int pm, double* pU,
                            double* pC, double* pDfct, unsigned int* pSn, unsigned int* pEn)
 {

    SPTree_LQueue::LoadNet( (cIndex)nmx,  (cIndex)mmx, (cIndex)pn,  (cIndex)pm, (cFRow) pU, (cCRow) pC,
                                (cFRow) pDfct, (cIndex_Set) pSn, (cIndex_Set) pEn );
 }
 unsigned int SPTree_LQueue::MCFmmax()
 {
    return (unsigned int) MCFClass::MCFmmax();
 }
 unsigned int SPTree_LQueue::MCFnmax()
 {
...
void SPTree_LQueue::SolveMCF( void )
{
 if( MCFt )
  MCFt->Start();
...
void SPTree_LQueue::MCFGetX( FRow F , Index_Set nms , cIndex strt , Index stp )
{
 SPTree_LQueue::MCFGetX( NDsts , DstBse , F , nms , strt , stp );

 }   // end( SPTree::MCFGetX )

/*--------------------------------------------------------------------------*/

void SPTree_LQueue::MCFGetRC( CRow CR , cIndex_Set nms , cIndex strt , Index stp )
{
 if( ! DirSPT )
...
inline MCFClass::CNumber SPTree_LQueue::MCFGetRC( cIndex i )
{
 if( ! DirSPT )
  throw( MCFException( "SPTree::MCFGetRC() not allowed if DirSPT == true" ) );
...
void SPTree_LQueue::MCFGetPi( CRow P , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
MCFClass::cCRow SPTree_LQueue::MCFGetPi( void )
{
 return( Pi + 1 );
 }
...
MCFClass::FONumber SPTree_LQueue::MCFGetFO( void )
{
 //#if( LABEL_SETTING )
 // if( FO == Inf<FONumber>() )
 //  FO = SPTree_LQueue::MCFGetFO( NDsts , DstBse );
 //#endif
...
void SPTree_LQueue::MCFArcs( Index_Set Startv , Index_Set Endv ,
...
void SPTree_LQueue::MCFCosts( CRow Costv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_LQueue::MCFUCaps( FRow UCapv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
  ...
  for( Index i = strt ; i < stp ; )
   *(UCapv++) = SPTree_LQueue::MCFUCap( i++ );
  }
 }  // end( SPTree::MCFUCaps )

/*--------------------------------------------------------------------------*/

void SPTree_LQueue::MCFDfcts( FRow Dfctv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_LQueue::ChgCosts( cCRow NCost , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt ) {
   nms++;
...
void SPTree_LQueue::ChgCost( Index arc , cCNumber NCost )
{
 if( DirSPT )
...
void SPTree_LQueue::ChgDfcts( cFRow NDfct , cIndex_Set nms , cIndex strt , Index stp )
{
...
void SPTree_LQueue::ChgDfct( Index nod , cFNumber NDfct )
{
 assert( false );  // not ready yet
...
void SPTree_LQueue::ChgUCaps( cFRow NCap , cIndex_Set nms , cIndex strt , Index stp )
{
 assert( false );
 }
...
void SPTree_LQueue::ChgUCap( Index arc , cFNumber NCap )
{
 assert( false );
 }
...
void SPTree_LQueue::CloseArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
void SPTree_LQueue::DelNode( cIndex name )
{
...
void SPTree_LQueue::OpenArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
MCFClass::Index SPTree_LQueue::AddNode( cFNumber aDfct )
{
...
void SPTree_LQueue::ChangeArc( cIndex name , cIndex nSS , cIndex nEN )
{
 #if( DYNMC_MCF_SPT )
  if( nSS < Inf<Index>() )
...
MCFClass::Index SPTree_LQueue::AddArc( cIndex Start , cIndex End , cFNumber aU ,
				cCNumber aC )
{
 #if( DYNMC_MCF_SPT )
...
void SPTree_LQueue::DelArc( cIndex name )
{
 SPTree_LQueue::CloseArc( name );  // limited implementation
 }
...
void SPTree_LQueue::ShortestPathTree( void )
{
...
inline void SPTree_LQueue::MCFGetX( Index ND , cIndex_Set DB , FRow F ,
...
MCFClass::FONumber SPTree_LQueue::MCFGetFO( Index ND , cIndex_Set DB )
{
 FONumber tFO = 0;
 for( Index i = ND ; i-- ; )
  tFO += B[ i ] * Pi[ DB[ i ] ];
...
inline void SPTree_LQueue::Initialize( void )
{
 status = MCFClass::kOK;
 FO = Inf<SPTree_LQueue::FONumber>();
...
inline void SPTree_LQueue::ScanFS( cIndex mi )
{
 cCNumber pmi = Pi[ mi ];
...
inline MCFClass::Index SPTree_LQueue::ExtractQ( void )
{
 Index mi;
...
inline void SPTree_LQueue::InsertQ( cIndex j , cCNumber label )
{
 //#if( ( SPT_ALGRTM == 0 ) || ( SPT_ALGRTM == 3 ) )  //- - - - - - - - - - - -
...
inline void SPTree_LQueue::CalcArcP( void )
{
 if( ! ReadyArcP ) {
...
inline MCFClass::Index SPTree_LQueue::Startn( cIndex What )
{
 // we search for:
...
inline void SPTree_LQueue::MemAlloc( void )
{
...
inline void SPTree_LQueue::MemDeAlloc( void )
{
 if( mmax && nmax ) {
  delete[] B;
  delete[] DstBse;
...
MCFClass::Index       SPTree_LQueue::InstCntr = 0;
MCFClass::Index       SPTree_LQueue::maxnmax = 0;
MCFClass::Index_Set   SPTree_LQueue::Stack = NULL;

#if( SPT_STRTN && SAME_GRPH_SPT )
 MCFClass::Index_Set  SPTree_LQueue::Startn = NULL;
#endif

#if( SAME_GRPH_SPT && ( ! DYNMC_MCF_SPT ) )
 MCFClass::Index_Set  SPTree_LQueue::StrtFS = NULL;

 MCFClass::Index_Set  SPTree_LQueue::Dict = NULL;
 MCFClass::Index_Set  SPTree_LQueue::DictM1 = NULL;
 MCFClass::bool       SPTree_LQueue::DirSPT = false;
#endif

/*--------------------------------------------------------------------------*/
/*-------------------------- End File SPTree_LQueue.C ----------------------*/
/*--------------------------------------------------------------------------*/
