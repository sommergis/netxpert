...
SPTree_Heap::SPTree_Heap( cIndex nmx , cIndex mmx , bool Drctd, int HeapAry )
	:
        MCFClass( nmx , mmx )
{
 DirSPT = Drctd;
...
void SPTree_Heap::LoadNet( cIndex nmx , cIndex mmx , cIndex pn , cIndex pm ,
...
// Johannes Sommer
 ...
 void SPTree_Heap::LoadNet(unsigned int nmx, unsigned int mmx, unsigned int pn, unsigned int pm, double* pU,
                            double* pC, double* pDfct, unsigned int* pSn, unsigned int* pEn)
 {

    SPTree_Heap::LoadNet( (cIndex)nmx,  (cIndex)mmx, (cIndex)pn,  (cIndex)pm, (cFRow) pU, (cCRow) pC,
                                (cFRow) pDfct, (cIndex_Set) pSn, (cIndex_Set) pEn );
 }
 unsigned int SPTree_Heap::MCFmmax()
 {
    return (unsigned int) MCFClass::MCFmmax();
 }
 unsigned int SPTree_Heap::MCFnmax()
 {
...
void SPTree_Heap::SolveMCF( void )
{
 if( MCFt )
  MCFt->Start();
...
void SPTree_Heap::MCFGetX( FRow F , Index_Set nms , cIndex strt , Index stp )
{
 SPTree_Heap::MCFGetX( NDsts , DstBse , F , nms , strt , stp );

 }   // end( SPTree::MCFGetX )

/*--------------------------------------------------------------------------*/

void SPTree_Heap::MCFGetRC( CRow CR , cIndex_Set nms , cIndex strt , Index stp )
{
 if( ! DirSPT )
...
inline MCFClass::CNumber SPTree_Heap::MCFGetRC( cIndex i )
{
 if( ! DirSPT )
  throw( MCFException( "SPTree::MCFGetRC() not allowed if DirSPT == true" ) );
...
void SPTree_Heap::MCFGetPi( CRow P , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
MCFClass::cCRow SPTree_Heap::MCFGetPi( void )
{
 return( Pi + 1 );
 }
...
MCFClass::FONumber SPTree_Heap::MCFGetFO( void )
{
 //#if( LABEL_SETTING )
  if( FO == Inf<FONumber>() )
   FO = SPTree_Heap::MCFGetFO( NDsts , DstBse );
 //#endif
...
void SPTree_Heap::MCFArcs( Index_Set Startv , Index_Set Endv ,
...
void SPTree_Heap::MCFCosts( CRow Costv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_Heap::MCFUCaps( FRow UCapv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
  ...
  for( Index i = strt ; i < stp ; )
   *(UCapv++) = SPTree_Heap::MCFUCap( i++ );
  }
 }  // end( SPTree::MCFUCaps )

/*--------------------------------------------------------------------------*/

void SPTree_Heap::MCFDfcts( FRow Dfctv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_Heap::ChgCosts( cCRow NCost , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt ) {
   nms++;
...
void SPTree_Heap::ChgCost( Index arc , cCNumber NCost )
{
 if( DirSPT )
...
void SPTree_Heap::ChgDfcts( cFRow NDfct , cIndex_Set nms , cIndex strt , Index stp )
{
...
void SPTree_Heap::ChgDfct( Index nod , cFNumber NDfct )
{
 assert( false );  // not ready yet
...
void SPTree_Heap::ChgUCaps( cFRow NCap , cIndex_Set nms , cIndex strt , Index stp )
{
 assert( false );
 }
...
void SPTree_Heap::ChgUCap( Index arc , cFNumber NCap )
{
 assert( false );
 }
...
void SPTree_Heap::CloseArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
void SPTree_Heap::DelNode( cIndex name )
{
...
void SPTree_Heap::OpenArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
MCFClass::Index SPTree_Heap::AddNode( cFNumber aDfct )
{
...
void SPTree_Heap::ChangeArc( cIndex name , cIndex nSS , cIndex nEN )
{
 #if( DYNMC_MCF_SPT )
  if( nSS < Inf<Index>() )
...
MCFClass::Index SPTree_Heap::AddArc( cIndex Start , cIndex End , cFNumber aU ,
				cCNumber aC )
{
 #if( DYNMC_MCF_SPT )
...
void SPTree_Heap::DelArc( cIndex name )
{
 SPTree_Heap::CloseArc( name );  // limited implementation
 }
...
void SPTree_Heap::ShortestPathTree( void )
{
...
inline void SPTree_Heap::MCFGetX( Index ND , cIndex_Set DB , FRow F ,
...
MCFClass::FONumber SPTree_Heap::MCFGetFO( Index ND , cIndex_Set DB )
{
 FONumber tFO = 0;
 for( Index i = ND ; i-- ; )
  tFO += B[ i ] * Pi[ DB[ i ] ];
...
inline void SPTree_Heap::Initialize( void )
{
 status = MCFClass::kOK;
 FO = Inf<SPTree_Heap::FONumber>();
...
inline void SPTree_Heap::ScanFS( cIndex mi )
{
 cCNumber pmi = Pi[ mi ];
...
inline MCFClass::Index SPTree_Heap::ExtractQ( void )
{
 Index mi;
...
inline void SPTree_Heap::InsertQ( cIndex j , cCNumber label )
{
 //#if( ( SPT_ALGRTM == 0 ) || ( SPT_ALGRTM == 3 ) )  //- - - - - - - - - - - -
...
inline void SPTree_Heap::CalcArcP( void )
{
 if( ! ReadyArcP ) {
...
inline MCFClass::Index SPTree_Heap::Startn( cIndex What )
{
 // we search for:
...
inline void SPTree_Heap::MemAlloc( void )
{
...
inline void SPTree_Heap::MemDeAlloc( void )
{
 if( mmax && nmax ) {
  delete[] B;
  delete[] DstBse;
...
MCFClass::Index       SPTree_Heap::InstCntr = 0;
MCFClass::Index       SPTree_Heap::maxnmax = 0;
MCFClass::Index_Set   SPTree_Heap::Stack = NULL;

#if( SPT_STRTN && SAME_GRPH_SPT )
 MCFClass::Index_Set  SPTree_Heap::Startn = NULL;
#endif

#if( SAME_GRPH_SPT && ( ! DYNMC_MCF_SPT ) )
 MCFClass::Index_Set  SPTree_Heap::StrtFS = NULL;

 MCFClass::Index_Set  SPTree_Heap::Dict = NULL;
 MCFClass::Index_Set  SPTree_Heap::DictM1 = NULL;
 MCFClass::bool       SPTree_Heap::DirSPT = false;
#endif

/*--------------------------------------------------------------------------*/
/*-------------------------- End File SPTree_Heap.C ------------------------*/
/*--------------------------------------------------------------------------*/
