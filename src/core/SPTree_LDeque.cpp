...
SPTree_LDeque::SPTree_LDeque( cIndex nmx , cIndex mmx , bool Drctd )
	:
        MCFClass( nmx , mmx )
{
 DirSPT = Drctd;
...
void SPTree_LDeque::LoadNet( cIndex nmx , cIndex mmx , cIndex pn , cIndex pm ,
...
// Johannes Sommer
 ...
 void SPTree_LDeque::LoadNet(unsigned int nmx, unsigned int mmx, unsigned int pn, unsigned int pm, double* pU,
                            double* pC, double* pDfct, unsigned int* pSn, unsigned int* pEn)
 {

    SPTree_LDeque::LoadNet( (cIndex)nmx,  (cIndex)mmx, (cIndex)pn,  (cIndex)pm, (cFRow) pU, (cCRow) pC,
                                (cFRow) pDfct, (cIndex_Set) pSn, (cIndex_Set) pEn );
 }
 unsigned int SPTree_LDeque::MCFmmax()
 {
    return (unsigned int) MCFClass::MCFmmax();
 }
 unsigned int SPTree_LDeque::MCFnmax()
 {
...
void SPTree_LDeque::SolveMCF( void )
{
 if( MCFt )
  MCFt->Start();
...
void SPTree_LDeque::MCFGetX( FRow F , Index_Set nms , cIndex strt , Index stp )
{
 SPTree_LDeque::MCFGetX( NDsts , DstBse , F , nms , strt , stp );

 }   // end( SPTree::MCFGetX )

/*--------------------------------------------------------------------------*/

void SPTree_LDeque::MCFGetRC( CRow CR , cIndex_Set nms , cIndex strt , Index stp )
{
 if( ! DirSPT )
...
inline MCFClass::CNumber SPTree_LDeque::MCFGetRC( cIndex i )
{
 if( ! DirSPT )
  throw( MCFException( "SPTree::MCFGetRC() not allowed if DirSPT == true" ) );
...
void SPTree_LDeque::MCFGetPi( CRow P , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
MCFClass::cCRow SPTree_LDeque::MCFGetPi( void )
{
 return( Pi + 1 );
 }
...
MCFClass::FONumber SPTree_LDeque::MCFGetFO( void )
{
 //#if( LABEL_SETTING )
 // if( FO == Inf<FONumber>() )
 //  FO = SPTree_LDeque::MCFGetFO( NDsts , DstBse );
 //#endif
...
void SPTree_LDeque::MCFArcs( Index_Set Startv , Index_Set Endv ,
...
void SPTree_LDeque::MCFCosts( CRow Costv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_LDeque::MCFUCaps( FRow UCapv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
  ...
  for( Index i = strt ; i < stp ; )
   *(UCapv++) = SPTree_LDeque::MCFUCap( i++ );
  }
 }  // end( SPTree::MCFUCaps )

/*--------------------------------------------------------------------------*/

void SPTree_LDeque::MCFDfcts( FRow Dfctv , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt )
   nms++;
...
void SPTree_LDeque::ChgCosts( cCRow NCost , cIndex_Set nms , cIndex strt , Index stp )
{
 if( nms ) {
  while( *nms < strt ) {
   nms++;
...
void SPTree_LDeque::ChgCost( Index arc , cCNumber NCost )
{
 if( DirSPT )
...
void SPTree_LDeque::ChgDfcts( cFRow NDfct , cIndex_Set nms , cIndex strt , Index stp )
{
...
void SPTree_LDeque::ChgDfct( Index nod , cFNumber NDfct )
{
 assert( false );  // not ready yet
...
void SPTree_LDeque::ChgUCaps( cFRow NCap , cIndex_Set nms , cIndex strt , Index stp )
{
 assert( false );
 }
...
void SPTree_LDeque::ChgUCap( Index arc , cFNumber NCap )
{
 assert( false );
 }
...
void SPTree_LDeque::CloseArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
void SPTree_LDeque::DelNode( cIndex name )
{
...
void SPTree_LDeque::OpenArc( cIndex name )
{
 #if( DYNMC_MCF_SPT )
  if( ! DirSPT )
...
MCFClass::Index SPTree_LDeque::AddNode( cFNumber aDfct )
{
...
void SPTree_LDeque::ChangeArc( cIndex name , cIndex nSS , cIndex nEN )
{
 #if( DYNMC_MCF_SPT )
  if( nSS < Inf<Index>() )
...
MCFClass::Index SPTree_LDeque::AddArc( cIndex Start , cIndex End , cFNumber aU ,
				cCNumber aC )
{
 #if( DYNMC_MCF_SPT )
...
void SPTree_LDeque::DelArc( cIndex name )
{
 SPTree_LDeque::CloseArc( name );  // limited implementation
 }
...
void SPTree_LDeque::ShortestPathTree( void )
{
...
inline void SPTree_LDeque::MCFGetX( Index ND , cIndex_Set DB , FRow F ,
...
MCFClass::FONumber SPTree_LDeque::MCFGetFO( Index ND , cIndex_Set DB )
{
 FONumber tFO = 0;
 for( Index i = ND ; i-- ; )
  tFO += B[ i ] * Pi[ DB[ i ] ];
...
inline void SPTree_LDeque::Initialize( void )
{
 status = MCFClass::kOK;
 FO = Inf<SPTree_LDeque::FONumber>();
...
inline void SPTree_LDeque::ScanFS( cIndex mi )
{
 cCNumber pmi = Pi[ mi ];
...
inline MCFClass::Index SPTree_LDeque::ExtractQ( void )
{
 Index mi;
...
inline void SPTree_LDeque::InsertQ( cIndex j , cCNumber label )
{
 //#if( ( SPT_ALGRTM == 0 ) || ( SPT_ALGRTM == 3 ) )  //- - - - - - - - - - - -
...
inline void SPTree_LDeque::CalcArcP( void )
{
 if( ! ReadyArcP ) {
...
inline MCFClass::Index SPTree_LDeque::Startn( cIndex What )
{
 // we search for:
...
inline void SPTree_LDeque::MemAlloc( void )
{
...
inline void SPTree_LDeque::MemDeAlloc( void )
{
 if( mmax && nmax ) {
  delete[] B;
  delete[] DstBse;
...
MCFClass::Index       SPTree_LDeque::InstCntr = 0;
MCFClass::Index       SPTree_LDeque::maxnmax = 0;
MCFClass::Index_Set   SPTree_LDeque::Stack = NULL;

#if( SPT_STRTN && SAME_GRPH_SPT )
 MCFClass::Index_Set  SPTree_LDeque::Startn = NULL;
#endif

#if( SAME_GRPH_SPT && ( ! DYNMC_MCF_SPT ) )
 MCFClass::Index_Set  SPTree_LDeque::StrtFS = NULL;

 MCFClass::Index_Set  SPTree_LDeque::Dict = NULL;
 MCFClass::Index_Set  SPTree_LDeque::DictM1 = NULL;
 MCFClass::bool       SPTree_LDeque::DirSPT = false;
#endif

/*--------------------------------------------------------------------------*/
/*-------------------------- End File SPTree_LDeque.C ----------------------*/
/*--------------------------------------------------------------------------*/
